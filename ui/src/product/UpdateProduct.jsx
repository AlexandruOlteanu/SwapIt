        ...
        const fetchData = async () => {
            try {
                ...
                if (encodedResponseTitle !== encodedTitle) {
                    const correctUrl = `/product/update/${encodedResponseTitle}/${productId}/`;
                    window.location.href = correctUrl;
                    return;
                }

                const categoryResponse = await ApiBackendService.getCategoryTree({ categoryId: productResponse.categoryId });
                let categoryLevel1 = '', categoryLevel2 = '', categoryLevel3 = '';
                categoryResponse.parentCategories.forEach((category, index) => {
                    if (index === 0) {
                        categoryLevel1 = category.value;
                    } else if (index === 1) {
                        categoryLevel2 = category.value;
                    } else if (index === 2) {
                        categoryLevel3 = category.value;
                    }
                });
                ...
                setProduct({
                    ...
                    productSpecifications: productResponse.productSpecifications.map(spec => ({ key: spec.key, value: spec.value })),
                    productImages: productResponse.productImages.map(image => ({ imageUrl: image.imageUrl }))
                });

                handleBlur();

                // Fetch Auth User Details
                const authUserDetails = await ApiBackendService.getAuthenticatedUserDetails({});
                if (authUserDetails.userId !== productResponse.userId) {
