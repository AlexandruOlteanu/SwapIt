...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query("SELECT p.productId as productId, p.title as title, p.price as price, p.popularity as popularity, " +
            "pi as productImages " +
            "FROM Product p LEFT JOIN p.productImages pi WHERE p.categoryId IN :categoriesIds ORDER BY RANDOM()")
    Page<ProductProjection> findAllRandomByCategoryIdIn(List<Integer> categoriesIds, Pageable pageable);

    @Query("SELECT p.productId as productId, p.title as title, p.price as price, p.popularity as popularity, " +
            "pi as productImages " +
            "FROM Product p LEFT JOIN p.productImages pi ORDER BY RANDOM()")
    @NotNull Page<ProductProjection> findAllRandom(Pageable pageable);

    @Query("SELECT p.productId as productId, p.title as title, p.price as price, p.popularity as popularity, " +
            "pi as productImages " +
            "FROM Product p LEFT JOIN p.productImages pi WHERE p.userId = :userId")
    Page<ProductProjection> findAllByUserId(Integer userId, Pageable pageable);

    @Query("SELECT p.productId as productId, p.title as title, p.price as price, p.popularity as popularity, " +
            "pi as productImages " +
            "FROM Product p LEFT JOIN p.productImages pi WHERE p.categoryId IN :categoriesIds")
    ...
    @Query("SELECT p.productId as productId, p.title as title, p.price as price, p.popularity as popularity, " +
            "pi as productImages " +
            "FROM Product p LEFT JOIN p.productImages pi")
